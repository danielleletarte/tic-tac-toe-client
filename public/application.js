...
/***/ function(module, exports, __webpack_require__) {
	...
	var gameStore = __webpack_require__(9);

	// Process kicked off wheb "New Game" button is clicked.
	var startNewGame = function startNewGame(event) {
	...
	var populateSquare = function populateSquare() {
	  ...
	  if (gameStore.game.clickCount % 2 === 1) {
	    ...
	    api.updateBoard(data).then(ui.newMoveSuccess);
	    ...
	    api.updateBoard(_data).then(ui.newMoveSuccess).catch(ui.newMoveFailure);
	    $('#status-text').text('Player x, your move!');
	  }
	};
	...
	var addNewGameHandler = function addNewGameHandler() {
	  $('#new-game-button').on('click', startNewGame);
	};

	module.exports = {
	  addHandlers: addHandlers,
	  addNewGameHandler: addNewGameHandler,
...
/***/ function(module, exports, __webpack_require__) {
	...
	var gameStore = __webpack_require__(9);
	var win = __webpack_require__(10);
	var api = __webpack_require__(11);
	...
	var newGameSuccess = function newGameSuccess(data) {
	  ...
	  api.grabStats().then(function (response) {
	...
	var newMoveSuccess = function newMoveSuccess(data) {
	  ...
	  if (win.winCheck(gameStore.game.cells) === 'x' || win.winCheck(gameStore.game.cells) === 'o') {
	    ...
	    var winner = win.winCheck(gameStore.game.cells);
	    ...
	    api.sendWinner(_data2);
	    lockBoard();
	  }
	};

	var lockBoard = function lockBoard() {
	  $('#0').off('click');
	  $('#1').off('click');
	  $('#2').off('click');
	  $('#3').off('click');
...
/***/ function(module, exports) {
	...
	var checkElementNull = function checkElementNull(element) {
	  return element !== '';
	};

	var winner = '';

	var winCheck = function winCheck(array) {
	  var boardArray = array;

	  // Build new boardArrays from boardArray for each winning combination

	  var winCombo1 = boardArray.slice(0, 3);
	  var winCombo2 = boardArray.slice(3, 6);
	  var winCombo3 = boardArray.slice(6, 9);
	  var winCombo4 = [boardArray.slice(0, 1).toString(), boardArray.slice(4, 5).toString(), boardArray.slice(8, 9).toString()];
	  var winCombo5 = [boardArray.slice(0, 1).toString(), boardArray.slice(3, 4).toString(), boardArray.slice(6, 7).toString()];
	  var winCombo6 = [boardArray.slice(1, 2).toString(), boardArray.slice(4, 5).toString(), boardArray.slice(7, 8).toString()];
	  var winCombo7 = [boardArray.slice(2, 3).toString(), boardArray.slice(5, 6).toString(), boardArray.slice(8, 9).toString()];
	  var winCombo8 = [boardArray.slice(2, 3).toString(), boardArray.slice(4, 5).toString(), boardArray.slice(6, 7).toString()];

	  // Check if current status of boarArray produces a winner

	  if (winCombo1.toString() === 'x,x,x' || winCombo1.toString() === 'o,o,o') {
	    winner = winCombo1[0];
	    if (winner === 'x') {} else if (winner === 'o') {}
...
/***/ function(module, exports, __webpack_require__) {
	...
	var grabStats = function grabStats() {
	  return $.ajax({
	    url: config.apiOrigin + '/games/',
	    method: 'GET',
	    headers: {
	...
	var sendWinner = function sendWinner(data) {
	  return $.ajax({
	    url: config.apiOrigin + '/games/' + gameStore.game.id,
	    method: 'PATCH',
	    headers: {
...
/***/ function(module, exports, __webpack_require__) {
	...
	var gameLog = __webpack_require__(14);
	var win = __webpack_require__(10);
	...
	var numberGames = function numberGames(data) {
	  var numberGames = gameLog.games.length - 1;
	  $('#games-played').text('Games Played : ' + numberGames);
	  return numberGames;
	};
	var numberWins = function numberWins(data) {
	  var winCount = 0;
	  var gameList = gameLog.games.map(function (e) {
	    return e.cells;
	  });
	  for (var i = 0; i < gameLog.games.length; i++) {
...
/***/ function(module, exports, __webpack_require__) {
	...
	var onPasswordReset = function onPasswordReset(event) {
	  event.preventDefault();
	  var data = getFormFields(this);
	  api.changePassword(data).then(ui.changePasswordSuccess);
	};

	var passwordResetConfirm = function passwordResetConfirm(event) {
